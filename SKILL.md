...
- `npm run delivery:check`: refresh batch acceptance, then write `outputs/_batch/delivery-readiness-report.json/md`; fail if accepted systems are smoke/local-e2e artifacts or if real pipeline evidence nodes did not complete.
...
Batch, `repair:batch`, and `repair:loop` automatically refresh `outputs/_batch/acceptance-report.json/md` at terminal state; run `npm run batch:acceptance` only for explicit re-checks after inspecting or moving artifacts. Treat `acceptance-report.json status=accepted` as the unattended delivery gate: every target system has a non-stale truth-readiness report, score >=95%, no uncovered writable claims, a pending-review/final whitepaper, and no remaining repair queue or follow-up blocker.
Before calling a real-system batch deliverable complete, run `npm run delivery:check`. Treat `delivery-readiness-report.json status=ready` as the final evidence audit: it confirms batch acceptance passed, each selected system came from a real pipeline output directory, smoke/local-e2e truth reports are absent, and required evidence/truth/narrative/fact-check nodes completed.
